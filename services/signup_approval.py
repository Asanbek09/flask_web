...
def user_approval_service(utype:int, model: Any) -> bool:
    try:
        ...
        if utype == 1:
            insert_admin(id=model.id, age=model.age, fname=model.firstname, lnam=model.lastname, position=model.position, emp_started=model.emp_date, emp_status=model.emp_status)
        elif utype == 2:
